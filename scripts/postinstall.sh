...
sudo -u $USER git -C $UHOME/lib/nixos remote set-url --push origin git@github.com:Yoran9111/nixos.git

# Install nixos configuration
mv /etc/nixos/configuration.nix /etc/nixos/bak.nix
ln -s $UHOME/lib/nixos/machines/nixOS1.nix /etc/nixos/configuration.nix
